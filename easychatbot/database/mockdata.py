...
def create_mockdata():
    chatbot = Chatbot(name='Nicole', match_threshold=0.6, 
    ...
    for qa_item in qas:
        ...
        for question in qa_item['questions']:
            db.session.add(Question(qa_id=qa.id, text=question))
        for answer in qa_item['answers']:
            db.session.add(Answer(qa_id=qa.id, text=answer))

    session_id = '509b1f4e-34c3-4aa2-968e-68d6aa0036df'
    messages = [
        Message(session_id=session_id, chatbot_id=chatbot.id, 
                text='Hello, how can I help you?', score=1.0, 
                created=datetime.strptime('2020-04-26 18:22:04.199418', '%Y-%m-%d %H:%M:%S.%f')),
        Message(session_id=session_id, user_id=user.id, 
                text='Hi, what\'s your name?', 
                created= datetime.strptime('2020-04-26 18:22:19.145471', '%Y-%m-%d %H:%M:%S.%f')),
        Message(session_id=session_id, chatbot_id=chatbot.id, 
                text=f'You can call me {chatbot.name}, nice to meet you.', score=0.62318558694474,
                created= datetime.strptime('2020-04-26 18:22:19.519452', '%Y-%m-%d %H:%M:%S.%f')),
        Message(session_id=session_id, user_id=user.id, 
                text='I\'m looking for the contact page?', 
                created= datetime.strptime('2020-04-26 18:22:44.837401', '%Y-%m-%d %H:%M:%S.%f')),
        Message(session_id=session_id, chatbot_id=chatbot.id, 
                text=f'Click here to go to the contact page.', score=0.23451616434673,
